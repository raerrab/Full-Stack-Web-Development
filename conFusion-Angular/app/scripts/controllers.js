...
.controller('MenuController', ['$scope', '$rootScope', 'menuFactory', 'favoriteFactory', 'cartFactory', function ($scope, $rootScope, menuFactory, favoriteFactory, cartFactory) {
    ...
    $scope.addTocart = function(dish) {
        ...
        console.log('Add to cart', dish._id);
        
        $rootScope.dishdetails.push(dish);
        //cartFactory.pushtocart(dish);  
        
        $rootScope.count = $rootScope.count + 1;
        $rootScope.countMessage='No. of Items {'+$rootScope.count+'}';
    };
}])
...
.controller('ShowCartController', ['$scope','$rootScope', '$location', 'submitOrderFactory', function ($scope, $rootScope, $location, submitOrderFactory) {

    $scope.items = $rootScope.dishdetails;
    
    i=0;
    $scope.total = 0; 
    while (i < $rootScope.count) {
        itemPrice = parseInt($rootScope.dishdetails[i].price);
    ...
    var dishes_obj = {dishes: [], sum:$scope.total/100};
    for(var i=0; i<$scope.items.length; i++){
        var dish = $scope.items[i];
        dishes_obj.dishes.push({
            _id: dish._id,
            name: dish.name,
            price: dish.price,
            image: dish.image
    ...
    $scope.submitOrder = function () {
        ...
        submitOrderFactory.save(dishes_obj);
        $rootScope.count=0;
        $rootScope.dishdetails = [];
        $rootScope.countMessage='No. of Items {'+$rootScope.count+'}';
...
.controller('HeaderController', ['$scope', '$state', '$rootScope', 'ngDialog', 'AuthFactory', 'cartFactory', function ($scope, $state, $rootScope, ngDialog, AuthFactory, cartFactory) {
    ...
    $scope.showCart = function() {
        var i=0;
        while (i<$rootScope.count) {
           console.log("Show Cart ", $rootScope.dishdetails[i].name);
            i++;
        }
    }
